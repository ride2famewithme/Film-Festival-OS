...
type VariantConfig = { [key: string]: { [key: string]: string } };

type DefaultVariants<V extends VariantConfig> = { [K in keyof V]?: keyof V[K] };

type VariantProps<V extends VariantConfig> = { [K in keyof V]?: keyof V[K] };

type CompoundVariant<V extends VariantConfig> = { [K in keyof V]?: keyof V[K] } & { className: string };
  ...
  return (props?: VariantProps<V> & { className?: string }) => {
    ...
    for (const compound of compoundVariants) {
      ...
      for (const [key, value] of Object.entries(conditions)) {
        ...
        if (propValue !== value) { matches = false; break; }
      }
      if (matches) compoundClasses.push(compoundClassName);
    }
